...
18.切换代理后等待20秒
19.改成检查country
20.同一代理使用三次，均失败，则更换下一行代理
21.加上大循环，所有任务执行完一次，再循环，用下一行数据执行
...
1.wait 时间 变成1-10秒随机 固定时间容易触发robot验证机制,wait 随机时间 在配置文件自己设置吧


2.断网检测机制，运行过程中如果断网的应对措施

3.记录弹出窗口的网址

4.页面加载时间过长，解决办法 刷新

5.程序出现很多的 <urlopen error [Errno 10061] >
解决办法 把发送http请求验证改成直接打开网页验证
