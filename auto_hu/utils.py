...
class Logger(object):
    ...
    def get_logger(self):
        """add handlers to logger,then return logger."""
...
selenium_logger = Logger('selenium.webdriver.remote.remote_connection', 'ERROR', 'ERROR').get_logger()


class ExcelReader(object):
    def __init__(self, datafile, sheet=0):
        """Read workbook"""
        self.book_name = os.path.abspath(datafile)
...
class FirefoxProfile(webdriver.FirefoxProfile):
    def __init__(self, profile_directory=None):
        """Rewrite class FirefoxProfile for making exe file."""
...
class Config:
    def __init__(self, conf):
        self.random_agent = conf['random_agent_spoofer'] if 'random_agent_spoofer' in conf else 'random_agent_spoofer.xpi'
        self.loop = conf['loop'] if 'loop' in conf else False
        self.interval = conf['interval'] if 'interval' in conf else [60, 120]
        self.datafile = conf['datafile'] if 'datafile' in conf else 'data.xls'
        self.proxytool = conf['proxytool'] if 'proxytool' in conf else 'AutoProxyTool.exe'
        self.proxycountry = conf['proxycountry'] if 'proxycountry' in conf else 'all'
